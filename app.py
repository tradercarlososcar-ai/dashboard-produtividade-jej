...
# ✅ CONEXÃO SEM CACHE (permite reconexão a cada rerun)
def iniciar_conexao():
...
st.sidebar.caption(f"⏰ Última atualização: {pd.Timestamp.now().strftime('%d/%m/%Y às %H:%M:%S')}")

# 4. BUSCA E CRUZAMENTO DE DADOS (EXECUTA A CADA RERUN)
try:
    ...
    if dados_fato:
        st.sidebar.info(f"✅ {len(dados_fato)} registros carregados do banco.")
    else:
        st.warning("⚠️ A consulta ao banco retornou vazia (0 registros).")
    # ---------------------------------
...
if dados_fato:
    ...
    meses_disp_num = (
        df_fato[df_fato['Ano'] == ano_sel]['Mês_Num'].unique().tolist() 
        if ano_sel != "Todos" 
        else df_fato['Mês_Num'].unique().tolist()
    )
    ...
    with col1:
        ...
        grafico_eq = alt.Chart(df_eq).mark_bar(color='#1f77b4').encode(
            x='Metros:Q',
            y=alt.Y('Funcionário:N', sort='-x')
        ).properties(height=300)
        st.altair_chart(grafico_eq, use_container_width=True)
        st.markdown("---")
        st.dataframe(
            df_eq.rename(columns={'Metros': 'Metros Totais'}),
            hide_index=True,
    ...
    with col2:
        ...
        grafico_mq = alt.Chart(df_mq).mark_bar(color='#ff7f0e').encode(
            x='Metros:Q',
            y=alt.Y('Máquina:N', sort='-x')
        ).properties(height=300)
        st.altair_chart(grafico_mq, use_container_width=True)
        st.markdown("---")
        st.dataframe(
            df_mq.rename(columns={'Metros': 'Metros Totais'}),
            hide_index=True,
            use_container_width=True
        )
    ...
    grafico_ob = alt.Chart(df_ob).mark_bar(color='#2ca02c').encode(
        x='Metros:Q',
        y=alt.Y('Obra:N', sort='-x')
    ...
    st.dataframe(
        df_resumo[['Data', 'nome_obra', 'producao_metros']].rename(
            columns={'nome_obra': 'Obra', 'producao_metros': 'Produção (m)'}
        ),
        hide_index=True,
        use_container_width=True
    )
    ...
    st.warning("⚠️ Nenhum dado encontrado na base.")